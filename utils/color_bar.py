...
class ColorBar(pg.GraphicsObject):
    ...
    def paint(self, p, *args):
        ...
        p.setPen(pg.QtGui.QColor(0, 0, 0, 255)) # 테두리색, 블랙
        p.setBrush(pg.QtGui.QColor(255, 255, 255, 255)) # 면색, 현재는 화이트 추후 글자색(화이트) 변경후 블랙으로 변경 필요
        p.drawRoundedRect(*(self.zone + (8.0, 8.0)))
        # p.drawRoundedRect(*(self.zone + (.0, .0)))
